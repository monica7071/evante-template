...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement("ALTER TABLE sales MODIFY COLUMN status ENUM('vacant','available','reserved','contract','installment','transfer','transferred') DEFAULT 'available'");
    ...
    {
        DB::statement("ALTER TABLE sales MODIFY COLUMN status ENUM('vacant','available','reserved','contract','installment','transfer','transferred') DEFAULT 'vacant'");

        DB::table('sales')->where('status', 'available')->update(['status' => 'vacant']);
        DB::table('sales')->where('status', 'transferred')->update(['status' => 'transfer']);

        DB::statement("ALTER TABLE sales MODIFY COLUMN status ENUM('vacant','reserved','contract','installment','transfer') DEFAULT 'vacant'");
    }
};
