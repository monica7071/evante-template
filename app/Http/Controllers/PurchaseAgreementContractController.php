...
class PurchaseAgreementContractController extends Controller
{
    public function preview(Sale $sale): StreamedResponse|RedirectResponse
    {
        return $this->streamDocument($sale, asAttachment: false);
    }

    public function download(Sale $sale): StreamedResponse|RedirectResponse
    {
        return $this->streamDocument($sale, asAttachment: true);
    }

    private function streamDocument(Sale $sale, bool $asAttachment): StreamedResponse|RedirectResponse
    {
        ...
        $template = PdfTemplate::where('contract_type', 'agreement_to_sell_and_purchase')
            ->with('mappings')
            ->latest()
            ->first();
        ...
        if (!$template) {
            return redirect()->route('buy-sale.index', ['status' => 'contract'])
                ->with('error', 'ยังไม่มี Agreement to Sell and Purchase Template กรุณาอัปโหลดก่อน');
        }
        ...
        $filename = 'purchase-agreement-' . $sale->id . '.pdf';
    ...
    private function buildContractData(SalePurchaseAgreement $agreement): array
    {
        ...
        // Map each installment by sequence: installment_1_amount_number, installment_1_date, etc.
